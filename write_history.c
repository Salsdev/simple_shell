...
int write_history(char *history)
{
	...
	char *tmp = NULL;
	ssize_t w, fd;
	int len = _lenstring(history);

	filename = _malloc(_lenstring(h) + _lenstring(".shell.history") + 4);
	_copystring(filename, h), _catstring(filename, "/.shell.history");
	...
	{
		tmp = _malloc(len + 4);
		_copystring(tmp, history), _catstring(tmp, "\n");
...
int print_history(void)
{
	...
	filename = _malloc(_lenstring(h) + _lenstring(".shell.history") + 4);
	_copystring(filename, h);
	_catstring(filename, "/.shell.history");
	...
	while (w > 0)
	{
		...
		if (w > 0)
		{
			...
			istr = printtoi(counter);
			_write(1, istr, _lenstring(istr)),	_write(1, "  ", 2);
			_write(1, NULL, 0), free(istr);
		}
		_write(-1, NULL, 0);
		while (w > 0 && *history != '\n')
		{
			_write(1, history, _lenstring(history));
			w = read(fd, history, 1);
		}
		if (w > 0 && *history == '\n')
			_write(1, "\n", 1);
