...
/**
 * addalias - add node to linked list
 * @head: first node
...
int addalias(alias **head, char *key, char *value)
{
	alias *node, *_head;
...
int print_lalias(alias *head)
{
	...
	while (head)
	{
		...
		_write(-1, NULL, 0);
		_write(1, head->key, _lenstring(head->key));
		_write(1, "='", 2);
		_write(1, head->value, _lenstring(head->value));
		_write(1, "'\n", 2);
		_write(1, NULL, 0);
...
int print_alias(alias *head, char *key)
{
	...
	{
		errno = -5;
		msg = _malloc(_lenstring("not found ") + _lenstring(key) + 4);
		_copystring(msg, "not found ");
		smn = _malloc(_lenstring("alias: ") + _lenstring(key) + 4);
		_copystring(smn, "alias: "), _catstring(smn, key);
	...
	while (head)
	{
		if (!_comp2string(head->value, key))
		{
			_write(-1, NULL, 0);
			_write(1, head->key, _lenstring(head->key));
			_write(1, "='", 2);
