...
void parse_args(char *cmd, const char *del, char ***args, int mod)
{
	...
	*args = NULL;
	tokenized = _tokstring(cmd, del, mod);
	while (tokenized != NULL)
	{
		...
		if ((tokenized[0] == '"' && tokenized[_lenstring(tokenized) - 1] == '"') ||
				(tokenized[0] == '\'' && tokenized[_lenstring(tokenized) - 1] == '\''))
		{
			trimmed_arg = _strdup(tokenized + 1);
			trimmed_arg[_lenstring(trimmed_arg) - 1] = '\0';
		}
		trim_spaces(&((*args)[index]), trimmed_arg);
		(*args)[index + 1] = NULL;
		if (trimmed_arg != tokenized)
			free(trimmed_arg);
		tokenized = _tokstring(NULL, del, mod);
		index++;
	}
}
